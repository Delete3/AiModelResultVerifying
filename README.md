...
開發時看模型用：把 `.stl`、`.ply`、`.obj`、`.tri` 模型，以及 `.pts` margin 線，拖曳到面板上的框或
右邊的 3D 畫面（可一次多個），每個檔案一列：

- 勾選框：顯示／隱藏；色塊：換顏色（預設依序給 8 個容易分辨的顏色）
- 透明度 0–100%；「線框」看網格；檔案本身有頂點顏色時，可勾「檔案內的顏色」改用它（預設不用，
  讓色塊和畫面上的顏色一致）
- 「置中」對準這個模型；「移除」
- 一行摘要：格式（TRI 會標 V1／V2）、頂點數與面數（STL 不列頂點數，因為每個三角形的角都各存一次）、
  外框尺寸 mm、檔案大小

`.pts` 的 margin 線另外有：

- 「線寬」1–10 px；「顯示在最上層」讓線不被模型擋住（例如看另一個座標系的線、或線跑進模型裡面時）
- 摘要列出點數、是否閉合、線長 mm（和 pipeline `margin.json` 的周長可以直接對照）
- 一個檔案有多個 `BEGIN_<fdi>` … `END_<fdi>` 區塊時，每個區塊畫成各自的線，不會把兩顆牙的線連起來
- 最後一點和第一點相同（ground truth 的寫法）或只差約一個間距時當作閉合；差很遠就畫成開放的線
- `[[x,y,z],…]` 或舊 job 的 `margin.json` 內容存成 `.pts` 也讀得到（和「牙冠設計」上傳 margin 用同一個 parser）

上方有「全部顯示／隱藏」「全部置中」「全部清除」。讀不了的檔案會列出原因，其他照常載入。

幾件設計上的事：

- **只在這個分頁看得到，病例不受影響。** 預覽模型畫在自己的圖層（three.js layer 1），攝影機在這個分頁
  只看這一層、在其他分頁只看 layer 0，所以切回「牙冠設計」時病例、margin、視角都和離開時一樣；
  預覽這邊也保有自己的視角。還在跑的 pipeline job 回來的牙冠也不會跑進預覽畫面。
- 在預覽分頁時 margin 編輯器暫停，點畫面不會在看不到的口掃上加點。
- 預覽分頁的縮放範圍放寬到 0.02–60 倍（牙冠設計是 0.4–8），小到螺絲、大到公分單位的模型都能看。
- PLY 沒有面（`element face 0`）就當點雲畫；OBJ 可以有多個物件和線段，會一起畫、一起調整。
